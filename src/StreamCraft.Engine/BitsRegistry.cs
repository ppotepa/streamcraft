...
internal class BitContext : IBitContext
{
    ...
    private readonly StreamCraft.Core.Messaging.IMessageBus _messageBus;
    ...
    public BitContext(
        ...
        StreamCraft.Core.Messaging.IMessageBus messageBus,
        IBit bit)
    {
        _engine = engine;
    ...
    public StreamCraft.Core.Messaging.IMessageBus MessageBus => _messageBus;
}

internal sealed class BitMessageBus : StreamCraft.Core.Messaging.IMessageBus
{
    private readonly StreamCraft.Core.Messaging.IMessageBus _inner;
    private readonly string _source;

    public BitMessageBus(StreamCraft.Core.Messaging.IMessageBus inner, IBit bit)
    {
        _inner = inner;
        _source = bit.Route?.Trim('/') ?? bit.Name;
    }

    public void Publish<TPayload>(StreamCraft.Core.Messaging.MessageType messageType, TPayload payload, StreamCraft.Core.Messaging.MessageMetadata? metadata = null)
    {
        _inner.Publish(messageType, payload, EnsureSource(metadata));
    }

    public Task PublishAsync<TPayload>(StreamCraft.Core.Messaging.MessageType messageType, TPayload payload, StreamCraft.Core.Messaging.MessageMetadata? metadata = null, CancellationToken cancellationToken = default)
    {
        return _inner.PublishAsync(messageType, payload, EnsureSource(metadata), cancellationToken);
    }

    public Guid Subscribe<TPayload>(StreamCraft.Core.Messaging.MessageType messageType, Action<TPayload> handler)
    ...
    private StreamCraft.Core.Messaging.MessageMetadata EnsureSource(StreamCraft.Core.Messaging.MessageMetadata? metadata)
    {
        if (metadata == null)
        {
            return StreamCraft.Core.Messaging.MessageMetadata.Create(source: _source);
        }
        ...
        return new StreamCraft.Core.Messaging.MessageMetadata
        {
