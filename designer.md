...
## UI framework modularization (Designer UI)

The Designer UI framework is now modular and extensible:

- **Control registry**: controls are registered via a registry so plugins can add/remove renderers without touching core.
- **Schema validation**: controls can define validation and defaults for robust props.
- **Diagnostics**: warnings/errors are surfaced through a diagnostics store + optional diagnostics panel.
- **Core utilities**: style/layout/drag are split into shared modules for reuse.

### Plugin model (UI controls)

- Register via `controlRegistry.register(name, renderer, { aliases, defaults, validate })`.
- Unregister via `controlRegistry.unregister(name)`.
- Batch register via `controlRegistry.registerMany([...])`.

### Diagnostics

- Enable/disable diagnostics at runtime.
- Use the diagnostics panel control for visibility in the UI.
