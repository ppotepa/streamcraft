...
export const Desktop: React.FC = () => {
    ...
    }, [canvas.items, layerMgmt.activeLayerId, layerMgmt.layers, overlayName, textStylesCaseFilter, textStylesCategoryId, textStylesCustomText, textStylesFavorites, textStylesFontSource, textStylesPreviewText, textStylesSearch, textStylesSelectedId, textStylesShadowFilter, textStylesSyncPreview, textStylesWeightFilter]);
    ...
    useEffect(() => {
        ...
        const payload: DockPrefs = {
            ...
            isDockCollapsed,
            dockedWindows,
            showLayersToolbox: windows.showLayersToolbox,
            showOverlayVideoPreview: windows.showOverlayVideoPreview,
            showDataSourceExplorer: windows.showDataSourceExplorer,
            showTextStyleEditor: windows.showTextStyleEditor,
            showSchedulerOverview: windows.showSchedulerOverview
        };
        try {
    ...
    }, [dockedWindows, isDockCollapsed, windows.showDataSourceExplorer, windows.showLayersToolbox, windows.showOverlayVideoPreview, windows.showSchedulerOverview, windows.showTextStyleEditor]);
    ...
    const isSchedulableItem = useCallback((item: CanvasItem) => {
        ...
        if (!source || itemOps.isSystemSource(source)) return false;
    ...
    useEffect(() => {
        const timer = window.setInterval(() => {
            const now = Date.now();
            if (canvas.isTransforming || canvas.transformHoldUntil.current > now) return;
    ...
    }, [canvas, isSchedulableItem, runTest]);
    ...
    const handleNewLayout = useCallback(() => {
        ...
        const getPreviewItemStyle = useCallback((item: CanvasItem) => {
        ...
        }, [canvas.items, imageDisplaySrc, resolveImageSource]);
        ...
        const addItem = (toolType: string, x: number, y: number, width: number, height: number) => {
            ...
            const created = canvas.addItem(toolType, x, y, width, height, layerMgmt.activeLayerId, layerMgmt.layers);
        ...
        const applyHistory = (index: number) => {
            ...
            canvas.setItems(entry.items);
            canvas.setSelectedIds(entry.selectedIds);
        ...
        const selectedItem = canvas.selectedIds.length > 0 ? canvas.items.find((item) => item.id === canvas.selectedIds[0]) ?? null : null;
        useEffect(() => {
        ...
        const extensionByTarget = useMemo(() => {
            ...
            extensions.uiExtensions.forEach((extension) => {
        ...
        const applyTextStyle = useCallback((style: TextStyleCatalogEntry) => {
            ...
            const nextTransform = style.textTransform === "uppercase" || style.textTransform === "lowercase" || style.textTransform === "none"
                ? style.textTransform
                : undefined;
            canvas.updateItem(selectedItem.id, {
        ...
        useEffect(() => {
            ...
            canvas.updateItem(selectedItem.id, { sourceId: undefined, endpointPath: undefined, fieldPath: undefined, scheduleIntervalMs: 0 });
        ...
        const tools = [
            ...
            UiText.desktop.tools.ellipse,
            UiText.desktop.tools.line,
            UiText.desktop.tools.polygon,
            UiText.desktop.tools.bind
        ];
        ...
        if (selectedItem) {
            contextBarCenter.push(
                contextField("Name", element("input", {
                    ...
                    onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { name: event.target.value })
                })),
                contextField("X", element("input", {
                    ...
                    onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { x: Number(event.target.value) || 0 })
                })),
                contextField("Y", element("input", {
                    ...
                    onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { y: Number(event.target.value) || 0 })
                })),
                contextField("W", element("input", {
                    ...
                    onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { width: Math.max(2, Number(event.target.value) || 0) })
                })),
                contextField("H", element("input", {
                    ...
                    onChange: (event: React.ChangeEvent<HTMLInputElement>) => {
                        ...
                        if (selectedItem.type === "line") {
                            canvas.updateItem(selectedItem.id, { height: nextHeight, strokeWidth: nextHeight });
                        } else {
                            canvas.updateItem(selectedItem.id, { height: nextHeight });
                        }
                    }
                }))
            );
            ...
            if (selectedItem.type === "text") {
                contextBarCenter.push(
                    ...
                    contextField("Text", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { label: event.target.value })
                    })),
                    contextField("Font", element(
                        "select",
                        {
                            ...
                            onChange: (event: React.ChangeEvent<HTMLSelectElement>) => {
                                ...
                                canvas.updateItem(textContextTarget.id, { fontFamily: event.target.value });
                            }
                        },
                        ...UiText.desktop.options.fonts.map((font) => element("option", { key: font, value: font }, font))
                    )),
                    contextField("Size", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => {
                            if (!textContextTarget) return;
                            canvas.updateItem(textContextTarget.id, { fontSize: Number(event.target.value) || 1 });
                        }
                    })),
                    element(
                        "button",
                        {
                            ...
                            onClick: () => {
                                if (!textContextTarget) return;
                                canvas.updateItem(textContextTarget.id, { fontWeight: isBold ? "normal" : "700" });
                            }
                        },
                        "B"
                    ),
                    element(
                        "button",
                        {
                            ...
                            onClick: () => {
                                if (!textContextTarget) return;
                                canvas.updateItem(textContextTarget.id, { fontStyle: isItalic ? "normal" : "italic" });
                            }
                        },
                        "I"
                    ),
                    contextField("Case", element(
                        "select",
                        {
                            ...
                            onChange: (event: React.ChangeEvent<HTMLSelectElement>) => {
                                if (!textContextTarget) return;
                                canvas.updateItem(textContextTarget.id, { textTransform: event.target.value as "none" | "uppercase" | "lowercase" });
                            }
                        },
                        ...UiText.desktop.options.transforms.map((transform) =>
                    ...
                    contextField("Color", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => {
                            if (!textContextTarget) return;
                            canvas.updateItem(textContextTarget.id, { textColor: event.target.value });
                        }
                    })),
                    contextField("Spacing", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => {
                            if (!textContextTarget) return;
                            canvas.updateItem(textContextTarget.id, { letterSpacing: Number(event.target.value) || 0 });
                        }
                    })),
                    element(
                        "button",
                        {
                            ...
                            type: "button",
                            onClick: () => windows.setShowTextStyleEditor(true)
                        },
            ...
            if (selectedItem.type === "image") {
                contextBarCenter.push(
                    ...
                    contextField("Image", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { src: event.target.value })
                    }))
                );
            }
            ...
            if (selectedItem.type === "progress") {
                contextBarCenter.push(
                    ...
                    contextField("Value", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { value: Number(event.target.value) || 0 })
                    })),
                    contextField("Min", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { minimum: Number(event.target.value) || 0 })
                    })),
                    contextField("Max", element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { maximum: Number(event.target.value) || 100 })
                    })),
                    contextField("Style", element(
                        "select",
                        {
                            ...
                            onChange: (event: React.ChangeEvent<HTMLSelectElement>) => canvas.updateItem(selectedItem.id, { progressStyle: event.target.value as "blocks" | "continuous" })
                        },
                        ...UiText.desktop.options.progressStyles.map((option) =>
            ...
            if (selectedItem.type === "rect" || selectedItem.type === "ellipse" || selectedItem.type === "line") {
                ...
                if (selectedItem.type !== "line") {
                    contextBarCenter.push(contextField("Fill", WF.Element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { fill: event.target.value })
                    })));
                }
                contextBarCenter.push(contextField("Stroke", WF.Element("input", {
                    ...
                    onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, { stroke: event.target.value })
                })));
                if (selectedItem.type === "line") {
                    contextBarCenter.push(contextField("Thickness", WF.Element("input", {
                        ...
                        onChange: (event: React.ChangeEvent<HTMLInputElement>) => canvas.updateItem(selectedItem.id, {
                ...
                contextBarCenter.push(
                    ...
                    WF.Element(
                        ...
                        hasBinding
                            ? WF.Element("button", {
                                className: "button context-bar-button",
                                onClick: () => canvas.updateItem(selectedItem.id, { sourceId: undefined, endpointPath: undefined, fieldPath: undefined, scheduleIntervalMs: 0 })
