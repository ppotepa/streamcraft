...
import { element, node, type FormChild } from "../../../forms/core";
...
export const createOverlayVideoPreviewDialog = (props: OverlayVideoPreviewDialogProps) => {
    ...
    const overlayLayer = element(
        "div",
        { className: "overlay-preview-layer" },
        props.showGrid ? element("div", { className: "overlay-preview-grid" }) : null,
        props.showOverlay
            ? element("div", { className: "overlay-preview-items" }, ...props.overlayNodes)
            : null
    );

    const videoArea = element(
        "div",
        {
            style:
        ...
        element(
            ...
            props.currentVideoUrl
                ...
                : element("div", { style: "color: #e0e0e0; font-size: 12px;" }, props.isLoading ? "Loading video..." : "No video selected.")
            ,
            overlayLayer
        )
    );
    ...
    const playlistContent = props.playlistCollapsed
        ...
        : element(
            ...
            element(
                ...
                element("span", null, isSearchMode ? "Search Results (Pexels)" : "Playlist (Cached Videos)"),
                element(
                    "button",
                    {
                        className: "canvas-properties-button",
                ...
                ...(props.videos.length > 0
                    ? props.videos.map((video) => {
                        ...
                        return element(
                            ...
                            element("div", { style: `width: 42px; height: 24px; border: 1px solid #9a9a9a; ${thumbStyle}` }),
                            element(
                                "div",
    ...
    const menuBar = node(
        ...
        node(
            ...
            { label: "Options" },
            node(ControlKind.menuItemEntry, { onClick: "clearOverlayVideoCache" }, "Clear Cache")
        )
    );
    ...
    const busyOverlay = props.isLoading && isSearchMode
        ? element(
            ...
            element(
                ...
                element("div", { style: "font-weight: 600; margin-bottom: 6px;" }, "Searching Pexels"),
                element(
                    "div",
                    { style: "display: flex; align-items: center; gap: 8px; font-size: 12px;" },
                    element("span", { className: "designer-status-spinner" }, "●"),
                    element("span", null, "Loading results...")
                )
            )
        )
        : null;
    ...
    return node(
        ...
        element(
            ...
            element(
                ...
                element("span", { className: "overlay-video-search-icon", "aria-hidden": "true" }),
                element("input", {
                ...
                props.statusMessage
                    ? element("span", { className: "overlay-video-search-status" }, props.statusMessage)
                    : null
                ,
                element(
                    "label",
                    ...
                    element("span", { className: "checkbox-text" }, "Show overlay")
                ),
                element(
                    "label",
                    { className: "checkbox-label overlay-video-toggle" },
                    ...
                    element("span", { className: "checkbox-text" }, "Grid")
                )
            ),
            element(
                "div",
