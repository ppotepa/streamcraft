...
const WidgetEditorModal: React.FC<{
  ...
  const endpointKey = endpoint ? `${endpoint.method}:${endpoint.path}` : "";
  const useDataSource = Boolean(draft?.sourceId && draft?.endpointPath);
          ...
          <section className="editor-section">
            ...
            <label className="field">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!enable) {
                    updateDraft({ sourceId: undefined, endpointPath: "", fieldPath: "", pollIntervalMs: undefined, template: undefined });
                  } else {
                    updateDraft({ sourceId: resolvedSourceId || sources[0]?.id, endpointPath: endpointKey || endpoints[0] ? `${endpoints[0]?.method}:${endpoints[0]?.path}` : "" });
                  }
                }}
              />
            </label>
            {useDataSource && (
              <>
                ...
                <label className="field">
                  <span>Template text (use {"{binding}"})</span>
                  <input
                    type="text"
                    value={draft.template || ""}
                    onChange={(e) => updateDraft({ template: e.target.value })}
                    placeholder="e.g. Latest: {binding} viewers"
                  />
                </label>
                <label className="field">
                  ...
                  <input
                    type="number"
                    min={0}
                    value={draft.pollIntervalMs ?? 0}
                    onChange={(e) => updateDraft({ pollIntervalMs: Number(e.target.value) || 0 })}
                  />
                </label>
          ...
          <section className="editor-section">
            ...
            <div className="layout-grid">
              ...
              <label className="field">
                ...
                <select
                  value={draft.fontWeight || "600"}
                  onChange={(e) => updateDraft({ fontWeight: e.target.value })}
                >
              ...
              <label className="field">
                ...
                <select
                  ...
                  onChange={(e) => updateDraft({ textAlign: e.target.value as WidgetProps["textAlign"] })}
                >
